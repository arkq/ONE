...
void convertResizeV11(const onnx::NodeProto &onnx_node, ConverterContext *context)
{
  ...
  if (scales_output)
  {
    auto *scales = dynamic_cast<mir::ops::ConstantOp *>(scales_output->getNode());
    assert(scales && "Scales could be a constant tensor only");
    auto scales_tensor = mir::Tensor<float>(scales->getValue());
    auto scales_tensor_elements = scales_tensor.getShape().numElements();
    ...
    if (scales_tensor_elements != 0)
    {
      if (scales_tensor_elements != rank)
        throw std::runtime_error{
          "Resize v11: The number of elements of 'scales' should be the same as the rank of input "
          "'X'"};
  ...
  // If scales is empty or not provided, use sizes to calculate scales.
  if (scales_vector.empty())
  {
    if (!sizes_output)
      throw std::runtime_error{
        "Resize v11: Sizes input is required when scales is empty or not provided"};

    auto *sizes = dynamic_cast<mir::ops::ConstantOp *>(sizes_output->getNode());
  ...
  auto result =
    createOp<mir::ops::ResizeOp>(graph, x, resize_method, scales_vector)->getOutput(0);
  context->setNodeOutputs(onnx_node, {result});
}
